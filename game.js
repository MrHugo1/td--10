...
const config = {
    ...
    towerTypes: [
        { id: 1, name: "Archer", cost: 50, damage: 15, range: 120, color: "#3498db", cooldown: 1000 },
        { id: 2, name: "Cannon", cost: 100, damage: 45, range: 100, color: "#e74c3c", cooldown: 2000 },
        { id: 3, name: "Ice", cost: 75, damage: 20, range: 90, color: "#1abc9c", cooldown: 1500 },
        { id: 4, name: "Barracks", cost: 150, range: 80, color: "#f39c12", isBarracks: true }
    ],
    enemyTypes: [
        { name: "Normal", health: 100, speed: 1, reward: 12, color: "#8e44ad", attackRange: 30, attackDamage: 10, attackCooldown: 1200 },
        { name: "Fast", health: 70, speed: 1.5, reward: 10, color: "#3498db", attackRange: 30, attackDamage: 8, attackCooldown: 900 },
        { name: "Tank", health: 200, speed: 0.7, reward: 25, color: "#e74c3c", attackRange: 35, attackDamage: 20, attackCooldown: 1800 }
    ],
    path: [
        {x: 180, y: 0},    // Giữa trên
        {x: 180, y: 80},   // Thẳng xuống giữa
        {x: 140, y: 80},   // Rẽ trái
        {x: 140, y: 250},  // Xuống trái
        {x: 220, y: 250},  // Sang phải
        {x: 220, y: 480},  // Xuống phải
        {x: 180, y: 480},  // Vào giữa
        {x: 180, y: 640}   // Xuống giữa dưới
    ]
};
...
function getPathPosition(progress) {
    ...
    let segmentLengths = [];
    for (let i = 0; i < config.path.length - 1; i++) {
        const dx = config.path[i + 1].x - config.path[i].x;
        const dy = config.path[i + 1].y - config.path[i].y;
        const length = Math.sqrt(dx * dx + dy * dy);
        totalLength += length;
        segmentLengths.push(length);
    }
    
    // Find which segment we're in based on progress
    let currentLength = 0;
    let segment = 0;
    ...
    for (let i = 0; i < segmentLengths.length; i++) {
        ...
        if (progress * totalLength <= nextLength) {
            segment = i;
    ...
    if (!config.path || !config.path[segment] || !config.path[segment + 1]) {
        return {
            x: config.path[config.path.length - 1].x,
            y: config.path[config.path.length - 1].y
        };
    }
...
function gameLoop(timestamp) {
    ...
    if (gameState.isPlaying && !gameState.gameOver) {
        ...
        if (!gameState.waveInProgress) {
            ...
            const waveBonus = 5 + (gameState.currentWave * 5);
        ...
        for (let tower of gameState.towers) {
            if (tower.isUpgrading) {
                tower.upgradeProgress += delta;
                if (tower.upgradeProgress >= tower.upgradeTime) {
                    tower.isUpgrading = false;
                    tower.upgradeLevel++;
                    tower.damage = tower.baseDamage * (1 + (tower.upgradeLevel * 0.1)); // Tăng 10% damage mỗi lần nâng cấp
                    tower.upgradeCost = tower.upgradeCost * 2; // Tăng gấp đôi chi phí cho lần nâng cấp tiếp theo
                }
            }
            
            // Chỉ cho phép tấn công nếu không đang nâng cấp
            if (!tower.isBarracks && !tower.isUpgrading) {
                if (tower.cooldownRemaining > 0) {
                    tower.cooldownRemaining -= delta * 1000;
                } else {
...
function draw() {
    ...
    for (let tower of gameState.towers) {
        ...
        // Vẽ nút nâng cấp nếu chưa nâng cấp tối đa và không phải barracks
        if (!tower.isBarracks && tower.upgradeLevel < 3) {
            const upgradeButtonX = tower.x + 25;
            const upgradeButtonY = tower.y;
            
            // Vẽ nút nâng cấp
            ctx.fillStyle = gameState.gold >= tower.upgradeCost ? '#2ecc71' : '#95a5a6';
            ctx.beginPath();
            ctx.arc(upgradeButtonX, upgradeButtonY, 8, 0, Math.PI * 2);
            ctx.fill();
            ...
            ctx.fillText('+', upgradeButtonX, upgradeButtonY);
            
            // Vẽ hiệu ứng nâng cấp nếu đang nâng cấp
            if (tower.isUpgrading) {
                const progress = tower.upgradeProgress / tower.upgradeTime;
                ctx.fillStyle = 'rgba(46, 204, 113, 0.3)';
                ctx.beginPath();
                ctx.arc(tower.x, tower.y, 12, -Math.PI/2, -Math.PI/2 + progress * Math.PI * 2);
                ctx.closePath();
                ctx.fill();
            }
        }
        
        // Hiển thị level nâng cấp nếu đã nâng cấp
        if (tower.upgradeLevel > 0) {
            ctx.fillStyle = '#2ecc71';
            ctx.font = '12px Arial';
            ctx.textAlign = 'center';
            ctx.fillText('+' + tower.upgradeLevel, tower.x, tower.y - 20);
        }
...
canvas.addEventListener('click', function(event) {
    ...
    // Kiểm tra click vào nút nâng cấp trước
    if (checkUpgradeButtonClick(x, y)) {
        return;
    }
...
// Thêm hàm mới để xử lý nâng cấp trụ
function upgradeTower(tower) {
    if (tower.isUpgraded || tower.isUpgrading || gameState.gold < tower.upgradeCost || tower.upgradeLevel >= 3) return;
...
// Thêm hàm kiểm tra click vào nút nâng cấp
function checkUpgradeButtonClick(x, y) {
    for (let tower of gameState.towers) {
        if (tower.isBarracks) continue; // Không cho nâng cấp trụ barracks
        
        // Tính toán vị trí nút nâng cấp (bên phải trụ)
        const upgradeButtonX = tower.x + 25;
        const upgradeButtonY = tower.y;
        const buttonRadius = 8;
        
        if (distance(x, y, upgradeButtonX, upgradeButtonY) < buttonRadius) {
            upgradeTower(tower);
            return true;
        }
    }
